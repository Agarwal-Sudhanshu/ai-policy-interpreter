...
import { PageContainer, PageHeader, Card, Button, inputClass } from "@/components/ui";
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        back={
          policyId
            ? { href: `/policies/${policyId}`, label: "← Back to Policy" }
            : { href: "/policies", label: "← Back to Policies" }
        }
      />

      {isGuest && questionsAsked !== null && (
        <div className="mb-4 rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm text-gray-700">
          ...
          {messages.map((m) =>
            m.role === "user" ? (
              ...
              <div key={m.id} className="flex justify-start">
                <div className="max-w-[85%] rounded-lg border border-gray-200 bg-gray-50 px-4 py-2 text-sm text-gray-900">
                  <p>{m.content}</p>
                  {m.source && (
