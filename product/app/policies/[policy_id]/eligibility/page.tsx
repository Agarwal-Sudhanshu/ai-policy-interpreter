            ...
            >
              <h2
                className={`mb-4 text-base font-semibold ${
                  result.eligible ? "text-emerald-800" : "text-red-800"
                }`}
              >
              ...
              {result.foirSkippedMessage && (
                <p className="mb-3 text-sm text-amber-800">{result.foirSkippedMessage}</p>
              )}
              <ul className="space-y-2 text-sm">
                {result.reasons.passed.map((r) => (
                  <li key={r} className="text-emerald-700">✓ {r}</li>
                ))}
